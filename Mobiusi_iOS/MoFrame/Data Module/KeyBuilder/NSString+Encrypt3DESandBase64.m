...
/*
 ...
 NSString *key = @"qz6ZnfpGTl1nvAhAymQoYLCk";
 NSString *src = @"atom|20141013180000"; //原文件内容
 NSString *encryptString = [src encryptStringWithKey:key];//加密结果
 NSString *decryptString = [encryptString decryptStringWithKey:key];//解密结果
...
{
    const void *vinitVec = (const void *) [_encryptVec UTF8String];
    
    size_t movedBytes  = 0;
    void * bufferPtr ;
    _encryptKey = (_encryptKey == nil)? [self get24BitKey:_encryptKey]: [self get24BitKey:_encryptKey];
...
- (NSString *)decryptStringWithKey:(NSString*)_encryptKey vec:(NSString *)_encryptVec
{
    ...
    CCCryptorStatus ccStatus = CCCrypt(kCCDecrypt,
                                       ...
                                       &movedBytes);
    if(ccStatus != 0){
        free(bufferPtr); //2015.8.11 new add fixed leak
        return @"";
    }
