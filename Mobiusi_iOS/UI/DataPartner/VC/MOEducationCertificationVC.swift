//
//  MOEducationCertificationVC.swift
//  Mobiusi_iOS
//
...
import Foundation
class MOEducationCertificationVC: MOBaseViewController {
    var educationPicture:MOAttchmentImageFileInfoModel?
    ...
    var step1View:MOUploadImageView = {
        let vi = MOUploadImageView(title: NSLocalizedString("请拍摄您的毕业证书或学位证书", comment: ""), subTitle: NSLocalizedString("请确保证件边框完整、字体清晰、亮度均匀", comment: ""), bgimageName: "icon_education_certification_bg")
    ...
    private func pickerPicture(){
        ...
        if (educationPicture != nil) {
            imagePickerVC.selectedAssets = [educationPicture?.imageAsset as Any]
        }
        ...
        imagePickerVC.didFinishPickingPhotosHandle = {[weak self] (photos,assets,isSelectOriginalPhoto) in
            ...
            educationPicture = model
            if let image =  photos?.first {
                step1View.configImage(selectedImage: image)
            }
    ...
    func uploadImage(){
        ...
        guard let educationPicture else {
            self.showMessage(NSLocalizedString("请拍摄您的毕业证书或学位证书", comment: ""))
            return
        }
        ...
        queue.async(group: taskGroup,execute: {
            ...
            if educationPicture.fileExtension.lowercased() == "png" {
                educationPicture.fileData = educationPicture.image?.pngData()
            }
            
            if educationPicture.fileData == nil {
                mineType = "image/jpeg"
                educationPicture.fileData = educationPicture.image?.jpegData(compressionQuality: 1.0)
            }
            
            MONetDataServer.shared().uploadFile(withFileName: educationPicture.fileName, fileData: educationPicture.fileData, mimeType: mineType) {dict in
                
                educationPicture.fileServerUrl = dict?["url"] as? String
                educationPicture.fileServerRelativeUrl = dict?["relative_url"] as? String
                taskGroup.leave()
        ...
        taskGroup.notify(queue: DispatchQueue.main, work: DispatchWorkItem(block: {
            ...
            MONetDataServer.shared().saveAuth(withAuthType: 4, identityCardFront: nil, identityCardBack: nil, driverLicenceMain: nil, driverLicenceDeputy: nil, workCompany: nil, workIncome: 0, educationImage: educationPicture.fileServerRelativeUrl, workType: 0) { dic in
                self.hidenActivityIndicator()
