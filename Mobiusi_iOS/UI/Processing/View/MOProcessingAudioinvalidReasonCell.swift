//
//  MOProcessingAudioinvalidReasonCell.swift
//  Mobiusi_iOS
//
//  Created by Mac on 2025/6/5.
//

import Foundation
class MOProcessingAudioinvalidReasonCell: MOTableViewCell {
	...
	lazy var titleLabel = {
		let label = UILabel(text: NSLocalizedString("无效原因", comment: ""), textColor: BlackColor, font: MOPingFangSCMediumFont(15))
	...
	func setupUI(){
		...
		textView.observeValue(forKeyPath: "contentSize") {[weak self] dict, object in
			let size:CGSize = dict["new"] as! CGSize
			   ...
			   if size.height != self.textView.bounds.height && size.height > 135{
				   textViewHeightdidChanged?(199 - 135 + size.height)
...
extension MOProcessingAudioinvalidReasonCell:UITextViewDelegate {
