//
//  MOPorcessingAudioSegmentCell.swift
//  Mobiusi_iOS
//
...
import Foundation
class MOPorcessingAudioSegmentCell: MOTableViewCell {
	
	
	var expandBtnDidClick:((_ isExpand:Bool)->Void)?
	...
	lazy var expandBtn = {
		...
		btn.setImage(UIImage(namedNoCache: "icon_black_arrow_d"), for: UIControl.State.selected)
	...
	func configCell(segmentData:MOAudioClipSegmentCustomModel,index:Int) {
		...
		expandBtn.isSelected = segmentData.isExpand
	}
	
	func addActions(){
		expandBtn.addTarget(self, action: #selector(expandBtnClick), for: UIControl.Event.touchUpInside)
	}
	
	@objc func expandBtnClick(){
		expandBtn.isSelected = !expandBtn.isSelected
		expandBtnDidClick?(expandBtn.isSelected)
	}
