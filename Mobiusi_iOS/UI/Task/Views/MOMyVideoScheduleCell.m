...
@implementation MOMyVideoScheduleCell
...
-(void)configeImageView:(UIImageView *)imageView imageIndex:(NSInteger)imageIndex model:(MOUserTaskDataModel *)data {
    ...
    if (fileCount >= imageIndex + 1) {
        ...
        if (model.cate == 2) {
            [imageView sd_setImageWithURL:[NSURL URLWithString:model.path]];
        }
        ...
        if (model.cate == 4) {
            if (model.thumbnailImage) {
                ...
                [self getVideoThumbnailFromURL:[NSURL URLWithString:model.path] completion:^(UIImage *thumbnail) {
                    model.thumbnailImage = thumbnail;
